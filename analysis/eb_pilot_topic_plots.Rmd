...
plot_grid(p1,p2,p3,nrow = 1,rel_widths = c(2,2,3))
```
...
plot_grid(p4,p5,nrow = 1,rel_widths = c(2,2.75))
pc3 <- pca[,"PC3"]
pc4 <- pca[,"PC4"]
x[pc3 > 2*pc4 + 0.58] <- "C"
```

Additional clusters are evident in the projection onto other
PCs.

```{r pca-plot-3, fig.width=7, fig.height=5}
...
plot_grid(p6,p7,p8,p9,nrow = 2,rel_widths = c(2,2.9))
```
...
For example, we can define a cluster, labeled "B1", that corresponds
closely to topic 2. The substructure here does not suggest clusters
that are as distinct as what we saw above, but because topic 2 is
picking up a very small subset of cells, it may be useful to define a
cluster so we can analyze it separately.
