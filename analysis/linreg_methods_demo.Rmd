...
`trimmed_lasso.m` to run the method. Here we load the results from
running the method:
...
parameter $k$, so here we take the setting of $k$ that is closest to
the true number of nonzero coefficients (which in this example is also
3).

As expected, most of the coefficient estimates are very sparse:

```{r plot-coef-trimmed-lasso, fig.height=2.5, fig.width=2.5}
plot_coefs(btrue,b)
```

For prediction, we need to estimate the intercept. Here we use the
MLE:
...
needed in general to get the right $k$, and cross-validation is not
implemented in the software, so we will have to do it ourselves.
